...
namespace dotnetgrad.tests
{
	public class MultiLayerPerceptronTests
	{
        ...
        [Test]
        public void MultiLayerPerceptronTests_Classifier_UsingSoftmax()
        {
            //Arrange
            var input1 = new List<double>() { 0.1, 0.8, -0.7 };
            var input2 = new List<double>() { -0.1, 1.0, 0.1 };
            ...
            var batch1 = new List<DataPoint>() {
                new DataPoint(input1, new List<double>() { 1.0, 0.0 }),
                new DataPoint(input2, new List<double>() { 0.0, 1.0 })
            };
            var batch2 = new List<DataPoint>() {
                new DataPoint(input1, new List<double>() { 1.0, 0.0 }),
                new DataPoint(input3, new List<double>() { 1.0, 0.0 })
            };
            var batch3 = new List<DataPoint>() {
                new DataPoint(input3, new List<double>() { 1.0, 0.0 }),
                new DataPoint(input2, new List<double>() { 0.0, 1.0 })
            ...
            var outputValues2 = mlp.FeedForward(input2);
            var outputValues3 = mlp.FeedForward(input3);
            Assert.That(outputValues1[0].Data, Is.EqualTo(1.0).Within(0.1));
            Assert.That(outputValues1[1].Data, Is.EqualTo(0.0).Within(0.1));
            Assert.That(outputValues2[0].Data, Is.EqualTo(0.0).Within(0.1));
            Assert.That(outputValues2[1].Data, Is.EqualTo(1.0).Within(0.1));
            Assert.That(outputValues3[0].Data, Is.EqualTo(1.0).Within(0.1));
            Assert.That(outputValues3[1].Data, Is.EqualTo(0.0).Within(0.1));
        }
    }
}
